...
# default: token generated by cozy initialisation for unique usage
# used: ''
registrationTokenSalt: String
# default: salt used to generate previous token
# used: ''
```
...
* __Security purpose:__ A token must be used to get reachable the first three onboarding steps, in a secured way and with a unique usage.
...
1. For ```welcome``` step:
    ...
    * __Token usage__: The registration token and salt will be provided as URL parameters, they must be recovered and sent into the save request in order to be accepted by the server.
...
3. For ```password``` step:
    ...
    * At the validation, properties ```registrationToken``` and ```registrationTokenSalt``` are removed (turned to '') from the ```user``` document (because unique usage).
...
#### Token usage for security and unique usage

The ```welcome```, ```agreement``` and ```password``` steps are used to get authentication user informations. Therefore, they handle sensitive informations in a non-authenticated way, so they need to be secured and used only once (by the affected user of course).

For this purpose, we will use __JSON Web Token__ to allow these steps to be processed only once and in a limited time. The URL will be unique, based on this token and reachable only while the token is valid. When the first three steps will be done, the token will be used and removed from the ```user``` document.

##### Generate a token using ```jsonwebtoken``` library :

```javascript
var jwt = require('jsonwebtoken');

var options = { /*..options properties..*/ };
var algorithm = { /*..algo property..*/ };
var salt = 'SECRET';

// Token generation
var token = jwt.sign(options, salt, algorithm);
```

> This token generation should be done outside of the Cozy, then only the token and the salt will be stored in the ```user``` document.

##### Verify the token when client request (server side):

```javascript
var jwt = require('jsonwebtoken');

// from request
var token = 'TOKENHASHHERE';
var salt = 'SECRET';

try {
  var decoded = jwt.verify(token, secretSalt);
  // authorize request / user data registration
} catch(err) {
  // reject request / user data registration
}
```

##### Registration steps URL based on the token:

The registration steps (```welcome```, ```agreement```, ```password```) must be reachable only by the user who has received the link. This is URL must be unique and computed from the registration token. Therefore, these steps will be available only during the registrationToken validity/existence.

So, these three steps will be reachable only via an URL computed with the correct token and secret key provided as parameters.
